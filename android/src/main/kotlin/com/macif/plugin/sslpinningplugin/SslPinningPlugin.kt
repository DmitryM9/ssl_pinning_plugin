...
class SslPinningPlugin : MethodCallHandler, FlutterPlugin {
    ...
    private fun handleCheckEvent(call: MethodCall, result: Result) {kljuj90h9uyt8776rr
        ...
        val issuerKey: String = arguments.get("issuerKey") as String
    ...
    fun checkConnexion(
        ...
        return if (issuerKey.isEmpty()) {
            val sha: String =
                this.getFingerprint(serverURL, timeout, httpHeaderArgs, type, httpMethod)
            allowedFingerprints.map { fp -> fp.toUpperCase().replace("\\s".toRegex(), "") }
                .contains(sha)
        } else {
            checkSecurity(issuerKey, serverURL, httpHeaderArgs, type, httpMethod)
        }
    }
    ...
    fun checkSecurity(
        ...
        val x509Tm: X509TrustManager = trustManagers[0] as X509TrustManager

        val issuers: Array<X509Certificate> = x509Tm.acceptedIssuers
        val trustedCertificates =
            issuers.filter { it.issuerX500Principal.name!!.contains(issuerKey) }
        val serverCertificates = httpClient.serverCertificates
        var result = false
        trustedCertificates.forEach { trustedCertificate ->
            serverCertificates.forEach { certificate ->
                try {
                    trustedCertificate.verify(certificate.publicKey)
                    result = true
                    this.hashString(type, certificate.encoded)
